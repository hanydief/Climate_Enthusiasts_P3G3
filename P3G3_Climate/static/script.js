...
function updateCharts(sample) {
  ...
  d3.json(myurl).then(function (data) {
    ...
    Plotly.update("temperatureChart", trace1, layout);
  });
}
...
function buildCharts(sample) {
  ...
  d3.json(myurl).then(function (data) {
    ...
    Plotly.newPlot("temperatureChart", my_data, layout);
  });
}
...
function init() {
  ...
  let dropdownurl = '/temperature/average_temperature'
...
// Add the second trace (scatter plot)
var trace2 = {
  x: [1, 2, 3, 4, 5],
  y: [16, 5, 11, 9, 7],
  mode: 'markers',
...
var layout = {
  title: 'Multiple Data on the Same Chart',
  xaxis: { title: 'X-axis' },
  yaxis: { title: 'Y-axis' }
};
